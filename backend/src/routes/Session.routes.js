...
const isoDate = (field, label, optional = false) => {
  ...
  return chain
    .isISO8601()
    .withMessage(`${label} must be a valid ISO 8601 datetime.`)
    .toDate();
};
...
const speakerValidation = [
  body("speakers")
    .optional()
    .isArray({ max: 10 })
    .withMessage("Speakers must be an array of up to 10 entries."),

  body("speakers.*.name")
    .if(body("speakers").exists())
    .trim()
    .notEmpty()
    .withMessage("Each speaker must have a name.")
    .isLength({ max: 100 })
    .withMessage("Speaker name must not exceed 100 characters."),

  body("speakers.*.title")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 150 })
    .withMessage("Speaker title must not exceed 150 characters."),

  body("speakers.*.company")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 100 })
    .withMessage("Speaker company must not exceed 100 characters."),

  body("speakers.*.bio")
    .optional({ nullable: true })
    .trim()
    .isLength({ max: 1000 })
    .withMessage("Speaker bio must not exceed 1000 characters."),

  body("speakers.*.avatar")
    .optional({ nullable: true })
    .trim()
    .isURL()
    .withMessage("Speaker avatar must be a valid URL."),

  body("speakers.*.userId")
    .optional({ nullable: true })
    .isMongoId()
    .withMessage("Speaker userId must be a valid MongoDB ID."),
];
...
const resourceValidation = [
  ...
  body("resources.*.label")
    ...
    .trim()
    .notEmpty()
    .withMessage("Each resource must have a label.")
    .isLength({ max: 100 })
    .withMessage("Resource label must not exceed 100 characters."),

  body("resources.*.url")
    .if(body("resources").exists())
    .trim()
    .notEmpty()
    .withMessage("Each resource must have a URL.")
    .isURL()
  ...
  body("resources.*.type")
    ...
    .trim()
    .isLength({ max: 30 })
    .withMessage("Resource type must not exceed 30 characters."),
];
...
const createSessionValidation = [
  ...
  body("location")
    .trim()
    .notEmpty()
    .withMessage("Session location or room is required.")
    .isLength({ max: 150 })
  ...
  body("streamUrl")
    ...
    .isLength({ max: 500 })
    .withMessage("Stream URL must not exceed 500 characters."),
];
...
const listQueryValidation = [
  ...
  query("location")
    ...
    .trim()
    .isLength({ max: 150 })
    .withMessage("Location filter must not exceed 150 characters."),
...
const myQueryValidation = [
  query("expoId")
    .optional()
    .isMongoId()
    .withMessage("expoId must be a valid MongoDB ID."),
];
...
router.get("/me/registrations", protect, myQueryValidation, getMyRegistrations);

router.get("/me/bookmarks", protect, myQueryValidation, getMyBookmarks);
router.get("/me/speaking", protect, getMySpeakingSessions);
...
router.get("/:id", optionalAuth, mongoId("id"), getSessionById);
...
router.post("/:id/bookmark", protect, mongoId("id"), toggleBookmark);
