...
import { z }                                  from 'zod';
import {
  ArrowLeft, Plus, Edit2, Trash2, Users,
  Clock, MapPin, Send, X, CheckCircle2,
...
const SESSION_FORMATS = [
  'keynote','panel','workshop','presentation','networking','demo','other',
];
...
function SessionFormPanel({ expoId, session, onClose, onSaved }) {
  ...
  const {
    ...
    defaultValues: session ? {
      ...
      format: 'presentation', isPublic: true, isFeatured: false,
    },
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        <form id="session-form" onSubmit={handleSubmit((v) => mutation.mutate(v))}
          noValidate className="flex flex-col gap-5">

          <Field label="Title" htmlFor="s-title" required error={errors.title?.message}>
            <input id="s-title" type="text" autoFocus {...register('title')}
              className={cn('input', errors.title && 'input-error')} />
          </Field>

          <div className="grid grid-cols-2 gap-4">
            <Field label="Format" htmlFor="s-format" required error={errors.format?.message}>
            ...
            <Field label="Room / Location" htmlFor="s-location" required error={errors.location?.message}>
              <input id="s-location" type="text" placeholder="e.g. Hall A"
                {...register('location')}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Start Time" htmlFor="s-start" required error={errors.startTime?.message}>
              <input id="s-start" type="datetime-local" {...register('startTime')}
                className={cn('input', errors.startTime && 'input-error')} />
            </Field>
            <Field label="End Time" htmlFor="s-end" required error={errors.endTime?.message}>
              <input id="s-end" type="datetime-local" {...register('endTime')}
          ...
          <Field label="Description" htmlFor="s-desc" error={errors.description?.message}>
            <textarea id="s-desc" rows={3} {...register('description')}
              className="input resize-none" placeholder="Optional session description…" />
          </Field>

          <Field label="Max Capacity" htmlFor="s-cap" error={errors.maxCapacity?.message}>
            <input id="s-cap" type="number" min={1} placeholder="Unlimited if blank"
              {...register('maxCapacity', { valueAsNumber: true })}
              className={cn('input', errors.maxCapacity && 'input-error')} />
          </Field>

          <Field label="Stream URL" htmlFor="s-stream" error={errors.streamUrl?.message}>
            <input id="s-stream" type="url" placeholder="https://…"
              {...register('streamUrl')} className={cn('input', errors.streamUrl && 'input-error')} />
          </Field>
      ...
      <div className="flex items-center justify-end gap-3 border-t border-outline-variant px-6 py-4">
        ...
        <button form="session-form" type="submit"
          disabled={isSubmitting || mutation.isPending} className="btn-secondary gap-2">
          {mutation.isPending ? (
            <>
            ...
            <><CheckCircle2 size={15} /> {isEdit ? 'Save Changes' : 'Create Session'}</>
          )}
        </button>
      </div>
    </motion.div>
...
export default function AdminSessions() {
  ...
  const [panel, setPanel]  = useState(null); // null | 'create' | session object
  ...
  const { data: expo } = useQuery({
    ...
    queryFn:  async () => { const { data } = await api.get(`/expos/${expoId}`); return data.data.expo; },
    staleTime: 5 * 60 * 1000,
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess:  () => { toast.success('Session deleted.'); setDelTarget(null); invalidate(); },
  ...
  const statusMutation = useMutation({
    ...
    onSuccess:  (_, { status }) => { toast.success(`Session marked as ${status}.`); invalidate(); },
    ...
    <>
      <div className="flex flex-col gap-6">
        ...
        <div className="page-header">
          ...
          <button onClick={() => setPanel('create')} className="btn-secondary gap-2">
                ...
                <tr>
                  <td colSpan={7} className="py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-body-sm text-on-surface-variant">Failed to load sessions.</span>
                ...
                <tr>
                  <td colSpan={7} className="py-16">
                    <div className="empty-state">
                      <div className="empty-state-icon mx-auto mb-3"><BookOpen size={24} /></div>
                      <h3 className="empty-state-title">No sessions yet</h3>
                      <p className="empty-state-body">
                        {search ? 'No sessions match your search.' : 'Create the first session for this expo.'}
                      </p>
                      {!search && (
                  ...
                  <tr key={session._id}
                    ...
                    <td className="px-4 py-density-high">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {session.isFeatured && (
                          <span className="font-mono text-label-sm text-secondary">⭐ Featured</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-density-high">
                    ...
                    <td className="px-4 py-density-high">
                      <div className="flex items-center gap-1">
                        ...
                        {session.status === 'scheduled' && (
                          <button
                            onClick={() => statusMutation.mutate({ id: session._id, status: 'live' })}
                            className="rounded p-1.5 text-on-surface-variant hover:bg-success-container
                                       hover:text-on-success-container transition-colors"
                            title="Go Live">
                            <Send size={14} />
                          </button>
                        )}
                        ...
                        {session.status === 'live' && (
                          <button
                            onClick={() => statusMutation.mutate({ id: session._id, status: 'completed' })}
                            className="rounded p-1.5 text-on-surface-variant hover:bg-surface-container
                        ...
                        <button
                          ...
                          className="rounded p-1.5 text-on-surface-variant hover:bg-error-container
        ...
        {panel && (
          <>
            ...
            <SessionFormPanel
              ...
              onSaved={() => { setPanel(null); invalidate(); }}
            />
          </>
        )}
      </AnimatePresence>
        ...
        {delTarget && (
          <div className="modal-overlay" role="dialog" aria-modal="true">
            ...
            >
              ...
              <h2 className="mb-1 text-headline-sm font-semibold text-on-surface">Delete session?</h2>
              <p className="mb-5 text-body-sm text-on-surface-variant">
                <span className="font-medium text-on-surface">"{delTarget.title}"</span> and all
                attendee registrations will be permanently deleted.
              </p>
              <div className="flex justify-end gap-3">
                <button onClick={() => setDelTarget(null)}
                  disabled={deleteMutation.isPending} className="btn-ghost">
                  Cancel
                </button>
                <button onClick={() => deleteMutation.mutate(delTarget._id)}
                  disabled={deleteMutation.isPending} className="btn-danger gap-1.5">
