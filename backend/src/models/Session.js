...
// ─── Enums ────────────────────────────────────────────────────────────────────
const SESSION_STATUSES   = Object.freeze(['scheduled', 'live', 'completed', 'cancelled']);
const SESSION_FORMATS    = Object.freeze(['keynote', 'panel', 'workshop', 'presentation', 'networking', 'demo', 'other']);
...
const SessionSchema = new mongoose.Schema(
  {
    ...
    format: {
      type:    String,
      enum:    { values: SESSION_FORMATS, message: `Format must be one of: ${SESSION_FORMATS.join(', ')}.` },
...
SessionSchema.index(
  ...
  { weights: { title: 10, tags: 4, location: 2, description: 1 }, name: 'session_text_search' }
);
...
SessionSchema.statics.detectConflicts = function (expoId, location, startTime, endTime, excludeId = null) {
  const query = {
    expoId,
    location,
          ...
          $cond: [
            { $gt: [{ $size: { $ifNull: ['$attendees', []] } }, 0] },
            {
              $multiply: [
                {
                  $divide: [
                    { $size: { $filter: { input: { $ifNull: ['$attendees', []] }, as: 'a', cond: '$$a.attended' } } },
