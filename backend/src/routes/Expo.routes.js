...
const {
  ...
  uploadExpoGallery,
  handleMulterError, 
} = require("../middleware/upload.middleware"); // ← Add
...
const dateField = (field, label) =>
  body(field)
    ...
    .isISO8601()
    .withMessage(
      `${label} must be a valid ISO 8601 date (e.g. 2025-09-01T09:00:00Z).`,
    )
    .toDate();
...
const createExpoValidation = [
  body("title")
    .trim()
    .notEmpty()
    .withMessage("Expo title is required.")
  ...
  body("floorPlanConfig")
    .notEmpty()
    .withMessage("Floor plan configuration is required.")
    .isObject()
    .withMessage("Floor plan configuration must be an object."),

  body("floorPlanConfig.rows")
    .notEmpty()
    .withMessage("Floor plan rows are required.")
    .isInt({ min: 1, max: 50 })
  ...
  body("floorPlanConfig.cols")
    .notEmpty()
    .withMessage("Floor plan columns are required.")
    .isInt({ min: 1, max: 50 })
  ...
  body("floorPlanConfig.boothWidth")
    .optional()
    .isFloat({ min: 1 })
    .withMessage("Booth width must be at least 1 metre.")
    .toFloat(),

  body("floorPlanConfig.boothHeight")
    .optional()
    .isFloat({ min: 1 })
    .withMessage("Booth height must be at least 1 metre.")
    .toFloat(),

  body("floorPlanConfig.aisleWidth")
    .optional()
    .isFloat({ min: 0 })
    .withMessage("Aisle width must be 0 or greater.")
    .toFloat(),
...
const updateFloorPlanValidation = [
  ...
  body("floorPlanConfig")
    .notEmpty()
    .withMessage("floorPlanConfig is required.")
    .isObject()
    .withMessage("floorPlanConfig must be an object."),
...
const listQueryValidation = [
  ...
  query("sort")
    ...
    .withMessage(
      "Sort must be one of: newest, oldest, start-asc, start-desc, title.",
    ),

  query("startFrom")
    .optional()
    .isISO8601()
    .withMessage("startFrom must be a valid ISO 8601 date."),

  query("startTo")
    .optional()
    .isISO8601()
    .withMessage("startTo must be a valid ISO 8601 date."),
...
// ─── Image Upload Routes ──────────────────────────────────────────────────
router.post("/:id/banner", mongoId("id"), uploadLimiter, uploadExpoBanner, handleMulterError, uploadBanner);
router.post("/:id/gallery", mongoId("id"), uploadLimiter, uploadExpoGallery, handleMulterError, uploadGallery);
router.delete("/:id/banner", mongoId("id"), deleteBanner);
router.delete("/:id/gallery/:imageId", mongoId("id"), deleteGalleryImage);
