...
const FloorPlanConfigSchema = new mongoose.Schema(
  {
    rows: {
      type: Number,
      required: true,
      min: [1, "Floor plan must have at least 1 row."],
      max: [50, "Floor plan cannot exceed 50 rows."],
    },
    cols: {
      type: Number,
      required: true,
      min: [1, "Floor plan must have at least 1 column."],
      max: [50, "Floor plan cannot exceed 50 columns."],
    },
...
// ─── Expo Status Enum ─────────────────────────────────────────────────────────
const EXPO_STATUSES = Object.freeze([
  "draft",
  "published",
  "ongoing",
  "completed",
...
const ExpoSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: {
        values: EXPO_STATUSES,
        message: `Status must be one of: ${EXPO_STATUSES.join(", ")}.`,
      },
      default: "draft",
    },
    ...
    boothPrice: {
      ...
      default: 0,
      min: 0,
      description: "Price per booth in cents",
    },
    boothCurrency: {
      type: String,
      default: "USD",
...
ExpoSchema.index(
  ...
  {
    weights: { title: 10, theme: 5, tags: 3, description: 1 },
    name: "expo_text_search",
  },
);
...
ExpoSchema.pre("save", function (next) {
  ...
  if (this.registrationDeadline && this.registrationDeadline > this.startDate) {
    return next(
      new Error("Registration deadline must be on or before the start date."),
    );
  }
